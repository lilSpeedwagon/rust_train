...
pub fn benchmark_set_pool_size(c: &mut Criterion) {
...
pub fn benchmark_set_pool_type(c: &mut Criterion) {
    let temp_dir = tempfile::TempDir::new().unwrap();
    let pool_types = ["none", "naive", "shared", "rayon"];
    let pool_size = 8;
    let client_thread_count = 4;
    let values_count = 100;
    let values_per_clinet = values_count / client_thread_count;

    let mut group = c.benchmark_group("kvs set pool type");
    group.sample_size(50);
    group.plot_config(PlotConfiguration::default());

    for pool_type in pool_types.iter() {
        group.bench_with_input(
            BenchmarkId::from_parameter(pool_type),
            pool_type,
            |b, &pool_type| {
                let _server_guard = run_server(
                    &temp_dir, pool_type, pool_size, &HOST.to_string(), PORT
                );
                    ...
                    for client_idx in 0..client_thread_count {
                        ...
                        let thread = std::thread::spawn(|| {
                            ...
                            for idx in idx_range {
                                ...
                                let cmd = models::Command::Set { key: key, value: value };
                                client.connect(
                                    HOST.to_string(), PORT, std::time::Duration::from_secs(10)
                                ).unwrap();
                                let response = client.execute_one(cmd, false).unwrap();
...
criterion_group!(benches, benchmark_set_pool_size, benchmark_set_pool_type);
